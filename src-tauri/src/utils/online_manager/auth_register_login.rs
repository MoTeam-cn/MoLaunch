        ...
        handler!(state, _app, _params, {
            ...
            if let Some(existing) = storage.load().await.unwrap_or(None) {
                if existing.is_registered() {
                    log_warn!(
                        "[Online] auth_register 拒绝: 设备已注册 (device_pk={})",
                        existing.device_pk
                    );
...
fn register_auth_login(d: &mut Dispatcher) {
    d.register(
        ...
        handler!(state, _app, _params, {
            ...
            log_info!("[Online] 设备登录成功: device_pk={}", updated.device_pk);
        ...
        handler!(state, _app, _params, {
            ...
            let creds = match existing {
                None => {
                    ...
                    let (req, mut new_creds) =
                        build_register_request(&kp, &device_id, &server_rsa_pem).map_err(|e| {
                ...
                Some(creds) => {
                    ...
                    if !creds.is_token_expired() {
                        ...
                        match super::refresh_credentials(&state, creds).await {
                            ...
                            Err(e) => {
                                log_warn!(
                                    "[Online] auth_init: refresh 续期失败: {}，尝试重新登录",
                                    e
                                );
            ...
            serde_json::to_value(AuthInitResult {
                status,
                error: None,
            })
            .map_err(|e| e.to_string())
