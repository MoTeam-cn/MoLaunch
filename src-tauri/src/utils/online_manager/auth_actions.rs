...
fn register_auth_refresh(d: &mut Dispatcher) {
    d.register("auth_refresh", handler!(state, _app, _params, {
        ...
        let updated = super::refresh_credentials(&state, creds).await.map_err(|e| {
            log_warn!("[Online] auth_refresh 续期失败: {}", e);
            e
        })?;

        serde_json::to_value(super::build_device_status(&updated, super::read_api_server_url(&state).await))
            .map_err(|e| e.to_string())
    }));
}
