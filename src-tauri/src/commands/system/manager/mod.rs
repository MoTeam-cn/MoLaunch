...
//! 使用 `utils::dispatcher::Dispatcher` 注册式分发，35 个 system action 在
//! `once_cell::sync::Lazy` 初始化时按域分组注册：game_dir / config / developer /
//! updater 及其它（about/logger/http_logs/certs/deeplink/ws）。非 Result 返回的
//! 命令需在 handler 内用 `serde_json::to_value` 包装。
...
use super::about::get_about_data;
use crate::handler;
