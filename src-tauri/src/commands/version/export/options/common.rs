//! 导出选项公共构建逻辑：选项汇总、目录探测、动态子选项扫描、glob 转义
//! 各分类选项（basic/misc/mods/packs/world）通过本模块复用。

use std::path::Path;

use super::super::types::ExportOption;
...
pub fn build_all_options(instance_dir: &Path) -> Vec<ExportOption> {
    ...
    super::basic::push(&mut opts, instance_dir, &rules_suffix);
    super::mods::push(&mut opts, instance_dir, &rules_suffix);
    super::packs::push(&mut opts, instance_dir, &rules_suffix);
    super::world::push(&mut opts, instance_dir, &rules_suffix);
    super::misc::push(&mut opts, instance_dir, &rules_suffix);
    opts
}

/// 检查实例目录下是否存在指定文件或目录之一
pub fn has_file_or_dir(instance_dir: &Path, names: &[&str]) -> bool {
    names.iter().any(|n| instance_dir.join(n).exists())
}

/// 扫描子文件夹/压缩包，生成动态子选项（用于资源包/存档/光影包）
pub fn scan_sub_options(
    instance_dir: &Path,
    folders: &[&str],
...
pub fn has_licence_file(instance_dir: &Path) -> bool {
