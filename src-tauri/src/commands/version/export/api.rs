//! 版本整合包导出逻辑实现（获取选项 / 执行导出 / 推进度事件 / 解析实例目录）
//!
//! 本文件承载 `export` 模块聚合入口（mod.rs）中夹带的实现逻辑：
//! `get_export_options`、`export_modpack`、`EXPORT_PROGRESS_EVENT` 及内部辅助函数。
//! 外部引用方通过 `export::*`（api 的 re-export）保持对外路径不变。
...
use super::scan::collect_export_files;
use super::types::{
...
pub async fn get_export_options(
    ...
    super::super::sanitize_version_id(&version_id)?;
    ...
    let opts = super::options::build_all_options(&instance_dir);
    Ok(opts)
}
...
pub async fn export_modpack(
    ...
    super::super::sanitize_version_id(&params.version_id)?;
    emit_progress(
        app,
        &params.version_id,
        ExportStage::Init,
    ...
    let (files, mut mod_infos) = if should_check_online {
        ...
        let result = super::network::check_mod_files_online(app, &files).await;
        match result {
            Ok(infos) => {
                log_info!(
    ...
    let zip_result = super::zip::build_modpack_zip(
        &instance_dir,
        &files,
        &mut mod_infos,
        &params,
