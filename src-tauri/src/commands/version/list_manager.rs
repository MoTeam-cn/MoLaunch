...
static DISPATCHER: Lazy<Dispatcher> = Lazy::new(|| {
    ...
    d.register(
        "get_version_loader_info",
        handler!(state, _app, params, {
            let p: VersionIdParams =
                serde_json::from_value(params).map_err(|e| format!("参数解析失败: {}", e))?;
            let (loader_type, loader_version) =
                list::get_version_loader_info(&state, p.version_id).await?;
    ...
    d.register(
        "check_local_modpack",
        handler!(state, _app, params, {
            let p: CheckLocalModpackParams =
                serde_json::from_value(params).map_err(|e| format!("参数解析失败: {}", e))?;
            let result = list::check_local_modpack(
                &state,
                p.manifest_hash,
                p.source,
                p.project_id,
    ...
    d.register(
        ...
        handler!(state, app, params, {
            let p: VersionIdParams =
                serde_json::from_value(params).map_err(|e| format!("参数解析失败: {}", e))?;
            list::repair_version_loader(&state, &app, &p.version_id).await
        }),
    );
    ...
    d.register(
        ...
        handler!(state, _app, params, {
            let p: SetSelectedVersionParams =
                serde_json::from_value(params).map_err(|e| format!("参数解析失败: {}", e))?;
    ...
    d.register(
        "update_version_personalization",
        handler!(state, _app, params, {
            let p: UpdatePersonalizationParams =
                serde_json::from_value(params).map_err(|e| format!("参数解析失败: {}", e))?;
