//! 认证模块统一分发逻辑（auth 域 meta_manager 的实现）
//! 28 个 auth action 在 `once_cell::sync::Lazy` 初始化时按域分组注册：
//! offline / microsoft / authlib 及会话通用操作。`ms_login_web_start` 等
//! 需要 `&app` 的 action 在对应分组内处理。

use once_cell::sync::Lazy;
use tauri::AppHandle;

use super::{authlib, microsoft, offline};
use crate::handler;
