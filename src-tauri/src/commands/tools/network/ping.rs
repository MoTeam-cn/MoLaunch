...
use super::super::types::{ServerPingParams, ServerPingResult};
...
async fn ping_server(host: &str, port: u16) -> Result<ServerPingResult, String> {
    ...
    let slp_fut = async {
        ...
        let online = players.get("online").and_then(|v| v.as_i64()).unwrap_or(0) as i32;
        let max = players.get("max").and_then(|v| v.as_i64()).unwrap_or(0) as i32;
        let version = status
            .get("version")
