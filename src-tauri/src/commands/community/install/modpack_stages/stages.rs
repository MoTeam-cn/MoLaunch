//! install_modpack 各阶段辅助实现（下载/解析/提取可选 Mod/复制 Logo）
...
pub async fn download_modpack_archive(
    state: &AppState,
...
pub fn parse_modpack_info(detected: &DetectedModpack) -> Result<ModpackInfo, String> {
...
///
/// 用于前端 preview 后弹窗显示。HMCL/MMC/MBBS 返回空列表（mods 已打包在 overrides 中）。
pub fn extract_optional_mods(info: &ModpackInfo) -> Vec<OptionalModInfo> {
    match info.format {
        ...
        // HMCL/MMC/MBBS/LauncherPack/Compress 整合包 mods 已打包在 overrides 中
        ModpackFormat::Hmcl
        | ModpackFormat::Mmc
        | ModpackFormat::Mcbbs
