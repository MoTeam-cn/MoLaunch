...
mod tests {
    ...
    #[test]
    fn test_parse_res_path_windows() {
        let uri = "https://res.localhost/web-common/wasm/cubiomes.wasm";
        assert_eq!(
            parse_res_path(uri),
            Some("web-common/wasm/cubiomes.wasm".to_string())
        );
    }

    #[test]
    fn test_parse_res_path_macos() {
        let uri = "res://localhost/web-common/wasm/cubiomes.wasm";
        assert_eq!(
            parse_res_path(uri),
            Some("web-common/wasm/cubiomes.wasm".to_string())
        );
    ...
    #[test]
    fn test_parse_res_path_with_query() {
        let uri = "https://res.localhost/web-common/wasm/cubiomes.wasm?v=123";
        assert_eq!(
            parse_res_path(uri),
            Some("web-common/wasm/cubiomes.wasm".to_string())
        );
    ...
    fn test_is_safe_path_valid() {
        assert!(is_safe_path("web-common/wasm/cubiomes.wasm"));
        assert!(is_safe_path("web-common/wasm/sub/foo.wasm"));
    }

    #[test]
    fn test_is_safe_path_traversal() {
        assert!(!is_safe_path("web-common/../secret.txt"));
        assert!(!is_safe_path("web-common/wasm/../../etc/passwd"));
    }

    #[test]
    fn test_map_to_resource_path() {
        assert_eq!(
            map_to_resource_path("web-common/wasm/cubiomes.wasm"),
            Some("wasm/cubiomes.wasm".to_string())
        );
        assert_eq!(
            map_to_resource_path("web-common/wasm/sub/foo.wasm"),
            Some("wasm/sub/foo.wasm".to_string())
        );
        assert_eq!(map_to_resource_path("invalid/path"), None);
    }

    #[test]
    fn test_guess_mime() {
        assert_eq!(guess_mime("cubiomes.wasm"), "application/wasm");
        assert_eq!(guess_mime("foo.js"), "application/javascript");
        assert_eq!(guess_mime("bar.png"), "image/png");
