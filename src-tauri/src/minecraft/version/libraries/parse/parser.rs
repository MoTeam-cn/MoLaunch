//! 库解析主逻辑：从版本 JSON 解析库 + 去重
//!
//! 规则推导见 rules，路径解析见 path。

use std::collections::HashMap;
use std::path::Path;

use super::super::LibEntry;
use super::path::{maven_natives_path, resolve_artifact};
use super::rules::{check_rules, is_native_matching_arch};
            ...
            if let Some(windows_name) = natives["windows"].as_str() {
                ...
                if let Some(cls) = library["downloads"]["classifiers"].get(&classifier) {
                    ...
                    let path = if let Some(p) = cls["path"].as_str() {
                        let Some(local) = super::path::artifact_local_path(p, game_dir) else {
                            continue;
                        };
                        local
                    } else {
