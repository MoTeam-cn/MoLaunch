//! 模糊匹配算法
//! 用于中文搜索本地映射：在 moddata.txt 中用中文关键词匹配对应的 CurseForge/Modrinth
//! Slug，把中文重写为英文关键词后再调平台 API。算法核心：基于最长公共子串的相似度，
//! 结合长度增益与位置邻近度加权，归一化后得到 0~1 区间分数。设计目标是对短查询
//! （如中文 Mod 名）有较高容错性，同时避免长字符串误匹配。
...
///
/// 基于最长公共子串的累积匹配长度，结合：
/// - 长度增益 `LEN_GAIN_BASE^(2+len)-OFFSET`：子串越长权重越高
/// - 位置邻近度：`1+STEP*max(0,RANGE-|qp-sp|)`，位置越近权重越高
/// - 归一化：`(累积增益/查询长度)*(NUMERATOR/sqrt(源长度+LEN_PAD))`
/// - 短查询补偿：查询长度 ≤2 时乘 `2.5-0.5*查询长度`
/// - `source`：源字符串；`query`：用户查询
