...
use super::types::{HashedMod, PreloadModInput, PreloadScope, PreloadUpdate};

/// 并发读元数据（仅 mods）+ 计算 hash
///
/// 限制并发度 8，每个条目在独立的 spawn_blocking 任务中：
/// 1. 若 `scope.read_jar_metadata`（mods）读 JAR 元数据；packs 为 zip 跳过，元数据留空
/// 2. 计算 CF MurmurHash2 + MR SHA1
/// 3. mods 立即 emit `{prefix}-preload-update` 元数据事件（project 字段为 None）
///
/// 返回所有成功处理的 `HashedMod`（任务 panic 的会被跳过并记日志）。
pub(crate) async fn read_jar_metadata_and_hash(
    ...
    for m in mods {
        ...
        let handle = tokio::task::spawn_blocking(move || {
            ...
            // 读元数据（mods 读 JAR，packs 为 zip 无元数据）
            let metadata = if scope.read_jar_metadata {
                crate::commands::version::mods::read_mod_metadata(&path)
            } else {
                Default::default()
            };
            ...
            // mods 立即 emit 元数据事件（前端马上能看到译名、logo、版本等）
            if scope.read_jar_metadata {
                let _ = app.emit(
    ...
    for h in handles {
        match h.await {
            ...
            Err(e) => crate::log_warn!("[Preload] 元数据读取任务失败: {}", e),
        }
    }
    hashed_mods
}
