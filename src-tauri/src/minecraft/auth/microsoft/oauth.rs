...
pub async fn exchange_auth_code(code: &str) -> Result<OAuthTokenResponse, MicrosoftLoginError> {
    ...
    if let Ok(body) = serde_json::from_str::<serde_json::Value>(&body_text) {
        if let Some(error) = body.get("error").and_then(|v| v.as_str()) {
            let desc = body
                ...
                .unwrap_or("");
            return Err(
                MicrosoftLoginError::new(format!("auth_code error: {} - {}", error, desc))
                    .with_step("auth_code")
                    .with_code(error),
            );
        }
    ...
    let token: OAuthTokenResponse = serde_json::from_str(&body_text).map_err(|e| {
        MicrosoftLoginError::new(format!("auth_code parse error: {}", e)).with_step("auth_code")
    })?;
...
pub async fn request_device_code() -> Result<DeviceCodeResponse, MicrosoftLoginError> {
    ...
    let params = [("client_id", config::OAUTH_CLIENT_ID), ("scope", ep.scope)];
    ...
    let result: DeviceCodeResponse = serde_json::from_str(&body_text).map_err(|e| {
        MicrosoftLoginError::new(format!("device_code parse error: {}", e)).with_step("device_code")
    })?;
...
pub async fn poll_device_code(
    ...
    if let Some(error) = body.get("error").and_then(|v| v.as_str()) {
        return match error {
            ...
            "authorization_declined" => Err(MicrosoftLoginError::new(format!(
                "poll declined: {}",
                body_text
            ))
            .with_step("poll")
            .with_code(error)),
            "expired_token" => Err(MicrosoftLoginError::new(format!(
                "poll expired: {}",
                body_text
            ))
            .with_step("poll")
            .with_code(error)),
            _ => Err(
                MicrosoftLoginError::new(format!("poll error: {}", body_text))
                    .with_step("poll")
                    .with_code(error),
            ),
        };
    ...
    if !status.is_success() {
        return Err(
            MicrosoftLoginError::new(format!("poll HTTP {}: {}", status, body_text))
                .with_step("poll"),
        );
    }

    let token: OAuthTokenResponse = serde_json::from_value(body).map_err(|e| {
        MicrosoftLoginError::new(format!("poll parse error: {}", e)).with_step("poll")
    })?;
...
pub async fn refresh_oauth_token(
    ...
    if let Ok(body) = serde_json::from_str::<serde_json::Value>(&body_text) {
        if let Some(error) = body.get("error").and_then(|v| v.as_str()) {
            let desc = body
                ...
                .unwrap_or("");
            return Err(
                MicrosoftLoginError::new(format!("refresh error: {} - {}", error, desc))
                    .with_step("refresh")
                    .with_code(error),
            );
        }
    ...
    let token: OAuthTokenResponse = serde_json::from_str(&body_text).map_err(|e| {
        MicrosoftLoginError::new(format!("refresh parse error: {}", e)).with_step("refresh")
    })?;
