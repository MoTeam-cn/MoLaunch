//! 下载进度状态机
//!
//! 集中管理 GlobalProgress 增量更新、速度滑动窗口与定期回调任务。

use super::super::types::GlobalProgress;
use std::sync::Arc;
use std::sync::Mutex as StdMutex;
use std::time::{Duration, Instant};
use tokio::sync::Mutex;

/// 下载进度状态机
pub(super) struct ProgressTracker {
    progress: Arc<StdMutex<GlobalProgress>>,
    speed_window: Arc<Mutex<std::collections::VecDeque<(u64, Instant)>>>,
    callback: Option<Arc<dyn Fn(GlobalProgress) + Send + Sync>>,
}

impl ProgressTracker {
    pub(super) fn new(
        total_files: usize,
        total_bytes: u64,
        callback: Option<Arc<dyn Fn(GlobalProgress) + Send + Sync>>,
    ) -> Self {
        Self {
            progress: Arc::new(StdMutex::new(GlobalProgress {
                total_files,
                total_bytes,
                is_active: true,
                ..Default::default()
            })),
            speed_window: Arc::new(Mutex::new(std::collections::VecDeque::new())),
            callback,
        }
    }

    /// 启动定期回调任务（滑动窗口速度计算 + 300ms 进度回调）
    pub(super) fn start_timer(&self) -> tokio::task::JoinHandle<()> {
        let prog = self.progress.clone();
        let sw = self.speed_window.clone();
        let cb = self.callback.clone();
        tokio::spawn(async move {
            let mut interval = tokio::time::interval(Duration::from_millis(300));
            loop {
                interval.tick().await;
                {
                    let p = prog.lock().unwrap();
                    if !p.is_active {
                        break;
                    }
                }
                // 在 timer 中 push 速度窗口，让下载过程中也能计算速度
                // 注意：StdMutex 的 MutexGuard 不是 Send，必须在 await 前释放
                let downloaded_snapshot = {
                    let p = prog.lock().unwrap();
                    p.downloaded_bytes
                };
                {
                    let mut window = sw.lock().await;
                    window.push_back((downloaded_snapshot, Instant::now()));
                    if window.len() > 10 {
                        window.pop_front();
                    }
                }
                let speed = {
                    let window = sw.lock().await;
                    if window.len() >= 2 {
                        let (first_bytes, first_time) = window.front().unwrap();
                        let (last_bytes, last_time) = window.back().unwrap();
                        let bytes_diff = last_bytes.saturating_sub(*first_bytes);
                        let time_diff = last_time.duration_since(*first_time).as_secs_f64();
                        if time_diff > 0.0 {
                            (bytes_diff as f64 / time_diff) as u64
                        } else {
                            0
                        }
                    } else {
                        0
                    }
                };
                let p_snapshot = {
                    let mut p = prog.lock().unwrap();
                    p.current_speed = speed;
                    p.clone()
                };
                if let Some(ref callback) = cb {
                    callback(p_snapshot);
                }
            }
        })
    }

    /// 任务完成（分片/单流下载过程中已增量更新 downloaded_bytes，这里不再重复加）
    pub(super) fn mark_completed(&self) {
        let mut p = self.progress.lock().unwrap();
        p.completed_files += 1;
    }

    /// 任务失败
    pub(super) fn mark_failed(&self) {
        let mut p = self.progress.lock().unwrap();
        p.failed_files += 1;
    }

    /// 任务跳过
    pub(super) fn mark_skipped(&self, skipped_size: u64) {
        let mut p = self.progress.lock().unwrap();
        p.skipped_files += 1;
        p.total_bytes = p.total_bytes.saturating_sub(skipped_size);
    }

    /// 推送一次进度回调
    pub(super) fn notify(&self) {
        let p = self.progress.lock().unwrap();
        if let Some(ref cb) = self.callback {
            cb(p.clone());
        }
    }

    /// 进度引用（供 downloader 增量更新 downloaded_bytes）
    pub(super) fn handle(&self) -> Arc<StdMutex<GlobalProgress>> {
        self.progress.clone()
    }

    /// 下载结束，标记为非活跃（定期回调任务在下个 tick 自行退出）
    pub(super) fn finish(&self) {
        let mut p = self.progress.lock().unwrap();
        p.is_active = false;
    }
}