...
fn test_is_mirror_url_official() {
    assert!(!is_mirror_url(
        "https://piston-meta.mojang.com/v1/packages/abc.json"
    ));
    assert!(!is_mirror_url(
        "https://resources.download.minecraft.net/abc/def"
    ));
    assert!(!is_mirror_url(
...
fn test_download_source_mode_from_str() {
    assert_eq!(
        DownloadSourceMode::from_str("official"),
        DownloadSourceMode::Official
    );
    assert_eq!(
        DownloadSourceMode::from_str("mirror"),
        DownloadSourceMode::Mirror
    );
    assert_eq!(
        DownloadSourceMode::from_str("smart"),
        DownloadSourceMode::Smart
    );
    // 未知值默认 Smart
    assert_eq!(
        DownloadSourceMode::from_str("unknown"),
        DownloadSourceMode::Smart
    );
