...
pub fn set_game_language(game_dir: &Path, version_id: &str, mc_version: &str) -> anyhow::Result<()> {
    ...
    if !options_path.exists() {
        ...
        log_info!("[Language] Created options.txt with lang={} for version {}", required_lang, version_id);
        return Ok(());
    }
    ...
    if !need_set_lang {
        log_info!("[Language] Language already set to {:?}, skipping", current_lang);
        return Ok(());
    }
    ...
    if let Some(ref current) = current_lang {
        if current == &required_lang {
            log_info!("[Language] Language is already {}, no change needed", required_lang);
            return Ok(());
        }
    }
    ...
    log_info!("[Language] Set language to {} for version {}", required_lang, version_id);
