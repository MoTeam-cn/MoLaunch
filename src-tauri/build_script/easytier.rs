//! 构建期检查 easytier-core / easytier-cli 嵌入式资源
//!
//! easytier-core / easytier-cli 通过 include_bytes! 在编译期嵌入二进制（见 resources.rs），
...
/// 当前打包的 easytier 版本（core/cli 同版本，仅供提示核对，不强制校验）
...
/// 返回 Some(files) 时列出 core、cli 与依赖 DLL 的相对路径。
fn platform_files() -> Option<Vec<String>> {
    let os = std::env::var("CARGO_CFG_TARGET_OS").unwrap_or_default();
    let arch = std::env::var("CARGO_CFG_TARGET_ARCH").unwrap_or_default();
    let (os_dir, arch_dir, core_name, cli_name, deps): (&str, &str, &str, &str, &[&str]) =
        match (os.as_str(), arch.as_str()) {
            ...
            ("linux", "x86_64") => ("linux", "x86_64", "easytier-core", "easytier-cli", &[]),
            ("linux", "aarch64") => ("linux", "aarch64", "easytier-core", "easytier-cli", &[]),
            ("macos", "x86_64") => ("macos", "x86_64", "easytier-core", "easytier-cli", &[]),
            ("macos", "aarch64") => ("macos", "aarch64", "easytier-core", "easytier-cli", &[]),
            _ => {
                println!(
    ...
    let mut files = vec![
        format!("easytier/{os_dir}/{arch_dir}/{core_name}"),
        format!("easytier/{os_dir}/{arch_dir}/{cli_name}"),
    ];
    files.extend(
        deps.iter()
...
/// 检查当前平台 easytier-core / easytier-cli 嵌入式资源是否存在；缺失时打印下载指引并终止构建
pub fn check_easytier() {
    ...
    if !missing.is_empty() {
        for f in &missing {
            println!(
                "cargo:warning=缺少 easytier 嵌入式资源: src-tauri/resources/{}",
                f
            );
        }
        println!(
            "cargo:warning=联机功能依赖 easytier-core/easytier-cli，请从 {} 下载 v{} 对应平台包，",
