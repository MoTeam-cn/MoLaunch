...
- **版本管理** — 原版 / Forge / Fabric / NeoForge / OptiFine 等載入器，多版本隔離、自訂 Java 與啟動參數
- **下載安裝** — Mod、資源包、整合包一鍵安裝（CurseForge / Modrinth），斷點續傳 + 中國境內鏡像加速
- **帳戶與皮膚** — 微軟 / 離線 / 外置登入（authlib-injector），多帳戶管理與皮膚 / 披風 3D 預覽
- **聯機** — 聯機大廳、搭橋聯機（EasyTier 虛擬組網 + 陶瓦聯機協定互通，免虛擬網卡）、FRP 內網穿透、紅石聯機（第三方隧道，免連接埠對應）
- **實用工具** — 啟動器資料遷移（PCL2 / HMCL / MultiMC / CurseForge 等）、種子地圖、NBT 編輯、存檔備份與還原、資源包編輯與轉換、Mod 依賴檢查與去重、伺服器狀態偵測、地址測速、Java 診斷、崩潰分析、創作指令生成等
- **AI 助手（實驗性）** — 對話式分析遊戲日誌與崩潰原因，附帶規則引擎日誌分析
- **外掛與個人化** — 首頁支援外掛模式與自訂版面（JSON / HTML / XML），內建快速統計、快取監控、系統監控等外掛
- **系統能力** — molaunch:// 深鏈喚醒、應用程式自動更新、快取管理、日誌檢視與開發者工具
