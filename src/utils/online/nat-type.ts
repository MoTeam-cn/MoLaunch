 ...
 * 参考 RFC 3489 的 NAT 分类与 PCL2/业界同类实现的判定逻辑：
 *
 * | host (本地) | srflx (公网反射) | 多个 srflx 端口 | NAT 类型            |
 * |------------|------------------|-----------------|---------------------|
 * | 有         | 无               | -               | Blocked / UDP 阻断  |
 * | 有         | 1 个             | -               | Cone NAT（默认全锥） |
 * | 有         | 多个             | 端口不同        | Symmetric（对称）   |
 * | 无         | -                | -               | Unknown（无候选）   |
 *
 * 注意：纯前端 WebRTC 无法精确区分 FullCone / RestrictedCone / PortRestrictedCone，
 * 统一归类为「Cone NAT」。若需要精确分类，需走 STUN RFC 5780 的多服务器探测，
 * 浏览器原生 API 不支持。当前实现满足联机可行性判断需求。
 */
