 ...
 * 外部下载 - 任务状态 / 进度切片
 *
 * 下载目录管理、进度（复用 versionStore）、启动/暂停/取消、已下载文件列表。
 * url/fileName 由来源切片持有并注入，供下载与文件名展示共用。
 */
...
import { isValidUrl } from './external-source'

export interface UseExternalDownloadTaskDeps {
  /** 下载地址（来源切片持有） */
  url: Ref<string>
  /** 下载文件名（来源切片持有） */
  fileName: Ref<string>
...
/** 外部下载任务切片 */
export function useExternalDownloadTask(deps: UseExternalDownloadTaskDeps) {
  const { url, fileName } = deps
  ...
  async function startDownload() {
    ...
    downloadFile(urlVal, nameVal)
      ...
      .catch((e) => {
        ...
        // 真实失败：后端已 mark_failed 重置 is_active，前端用 showModal + onConfirm 退出下载页
        showModal({
          type: 'error',
          title: '下载失败',
          message: msg,
