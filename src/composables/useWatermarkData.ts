 ...
 * 水印数据 composable
 *
 * 提供水印组件所需的设备 ID、版本号、构建指纹、当前时间戳等数据：
 * - 设备 ID：来自 SDK store，去除 `mcsdk-` 前缀后展示
 * - 版本号 + 构建指纹：来自 version.ts
 * - 屏印哈希：将「设备ID + 版本号 + 当前小时」拼接后哈希，作为隐写标识
 *
 * 屏印哈希的设计：
 * - 同一设备同一小时内的截图/拍照水印哈希一致，便于批量追溯
 * - 跨小时变化，避免攻击者通过比对截图剥离水印规律
 * - 不直接包含设备 ID（攻击者从截图无法逆推设备 ID），但服务端持有
 *   「设备ID → 屏印哈希」映射表，可反查
 */
