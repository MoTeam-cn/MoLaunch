 ...
 * 插件权限元信息表
 *
 * 定义所有可用权限的说明、用处、风险等级和参数要求。
 * 作为权限说明的单一数据源，供 SettingsPlugins.vue 展示和文档生成复用。
 *
 * 权限分类：
 * - 始终允许（alwaysAllowed=true）：emit / log，无需在 manifest 声明
 * - 普通权限：只读数据访问，低风险
 * - 高级权限：子进程执行等，高风险，需要额外的 processPermissions 配置
 */
