<template>
  ...
  >
    ...
    <div class="space-y-4 text-sm leading-relaxed text-gray-600">
      ...
      <!-- 联机功能说明（Scaffolding 兼容 / 联机大厅 / P2P / EasyTier / FRP） -->
      <div v-if="kind === 'online'">
        ...
        <div class="space-y-2 rounded-md border border-gray-200 bg-gray-50 px-3 py-2.5 text-xs leading-relaxed text-gray-600">
          <p>MoLaunch 联机模块基于与「陶瓦联机」相同的 Scaffolding 联机协议与 EasyTier 虚拟局域网独立开发，两者互相兼容：基于该协议实现的启动器（如陶瓦联机、PCL 等）获得的房间码可直接在 MoLaunch 中加入，反之亦然。</p>
          <p>除房间码联机外，MoLaunch 额外提供「联机大厅」：可在大厅中按整合包浏览当前开放的公开房间，无需输入房间码即可加入，方便与陌生人一起游玩（对方未设置密码且欢迎时）。</p>
          <p>优点：与其他基于陶瓦联机（Scaffolding）的启动器房间码互通，可加入任意协议兼容的房间；联机大厅按整合包找房，发现并加入陌生玩家开放房间更便捷。</p>
          <p>限制：房间内的部分增强能力仅对 MoLaunch 启动器玩家生效。例如房主变更 MC 端口时，端口热更新仅 MoLaunch 玩家能自动感知并继续游玩；使用其他启动器联机的玩家无法感知新端口，需要退出房间后重新加入才能进入游戏。</p>
          <p>联机使用 P2P 技术：通过国内外服务商提供的 TURN 服务器获取本机网络类型，并在无法直连时中转流量；同时创建一个虚拟网络用于游戏数据互通（无需安装虚拟网卡、无需管理员权限）。</p>
      ...
      <div v-else>
        ...
        <div class="space-y-2 rounded-md border border-gray-200 bg-gray-50 px-3 py-2.5 text-xs leading-relaxed text-gray-600">
          <p>实验性功能（AI 聊天、日志分析、模组翻译等）处于测试阶段，功能可能不稳定，界面与行为可能随时调整，数据可能随时丢失。</p>
          <p>AI 聊天与模组翻译会将对话内容、待翻译文本发送至您自行配置的模型服务端点（本地或第三方），数据外发与隐私风险由您自行评估与承担。</p>
          <p>模组翻译受 AI 大模型幻觉影响，翻译准确率与可用性可能不尽如人意，我们会持续优化提示词；建议优先使用更稳定、更完善的 i18n 翻译模组或资源包，本功能仅作为其尚未翻译或翻译较少模组的备选方案。</p>
          <p>模组翻译会修改并重打包 JAR 文件，含签名文件的模组重打包后签名将失效；请在使用前备份原文件，因翻译结果或重打包导致的任何问题由您自行承担。</p>
