...
[![Version](https://img.shields.io/badge/version-0.3.8--rc1-blue.svg)](https://github.com/MoTeam-cn/MoLaunch)
...
- **版本管理** — 原版 / Forge / Fabric / NeoForge / OptiFine 等加载器，多版本隔离、自定义 Java 与启动参数
- **下载安装** — Mod、资源包、整合包一键安装（CurseForge / Modrinth），断点续传 + 国内镜像加速
- **账户与皮肤** — 微软 / 离线 / 外置登录（authlib-injector），多账号管理与皮肤 / 披风 3D 预览
- **联机** — 联机大厅、搭桥联机（EasyTier 虚拟组网 + 陶瓦联机协议互通，免虚拟网卡）、FRP 内网穿透、红石联机（第三方隧道，免端口映射）
- **实用工具** — 启动器数据迁移（PCL2 / HMCL / MultiMC / CurseForge 等）、种子地图、NBT 编辑、存档备份与恢复、资源包编辑与转换、Mod 依赖检查与去重、服务器状态检测、地址测速、Java 诊断、崩溃分析、创作指令生成等
- **AI 助手（实验性）** — 对话式分析游戏日志与崩溃原因，附带规则引擎日志分析
- **插件与个性化** — 首页支持插件模式与自定义布局（JSON / HTML / XML），内置快速统计、缓存监控、系统监控等插件
- **系统能力** — 应用自更新、缓存管理、日志查看与开发者工具
...
- **[Scaffolding-MC](https://github.com/Scaffolding-MC/Scaffolding-MC)** — Minecraft 联机客户端数据交换协议（基于 EasyTier），定义房间码、联机网络与联机信息获取等标准流程；MoLaunch 联机模块基于与其一致的规范与 EasyTier 独立实现，与其他基于陶瓦联机（Scaffolding）的启动器房间码互通
...
- **[EasyTier](https://github.com/EasyTier/EasyTier)** — 点对点虚拟组网内核，搭桥联机 / 联机大厅的底层网络引擎（免虚拟网卡）
