...
- **バージョン管理** — バニラ / Forge / Fabric / NeoForge / OptiFine などのローダーに対応、バージョンごとに分離、Java と起動引数をカスタマイズ
- **ダウンロードとインストール** — Mod、リソースパック、Modパックをワンクリックインストール（CurseForge / Modrinth）、レジューム対応 + 中国国内ミラー高速化
- **アカウントとスキン** — Microsoft / オフライン / 外部ログイン（authlib-injector）、複数アカウント管理、スキン / ケープの 3D プレビュー
- **オンラインマルチプレイ** — ルームロビー、EasyTier 仮想ネットワークによる連携プレイ（Scaffolding プロトコル互換、仮想 NIC 不要）、NAT 越え FRP トンネル、第三者トンネル方式のレッドストーン連携（ポート開放不要）
- **ユーティリティ** — ランチャーデータ移行（PCL2 / HMCL / MultiMC / CurseForge など）、シードマップ、NBT エディタ、ワールドバックアップと復元、リソースパック編集・変換、Mod 依存関係チェックと重複排除、サーバー状態チェック、遅延計測、Java 診断、クラッシュ解析、コマンド・レシピ生成など
- **AI アシスタント（実験的）** — ゲームログとクラッシュ原因を対話形式で分析、ルールエンジンによるログ分析も内蔵
- **プラグインとカスタマイズ** — ホーム画面でプラグインモードとカスタムレイアウト（JSON / HTML / XML）に対応、クイック統計・キャッシュモニター・システムモニターなどの内蔵プラグイン
- **システム機能** — molaunch:// ディープリンク、自動アップデート、キャッシュ管理、ログビューア、開発者ツール
