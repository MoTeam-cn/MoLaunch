...
- **资源包编辑器文件树搜索过滤（M4）**（[filterTree.ts](src/utils/resourcepack/filterTree.ts)（新增） / [ResourcePackEditor.vue](src/views/tools/data/ResourcePackEditor.vue)）：文件树顶部新增搜索框——按关键字实时过滤（匹配文件名与相对路径，不区分大小写），目录仅保留含命中子孙的节点；搜索时自动展开命中路径的全部祖先目录，清空关键字恢复原展开状态；无命中时显示「未找到匹配的文件」空态。过滤逻辑抽为独立纯函数模块（`normalizeKeyword` / `filterTreeNode` / `collectExpandPaths`），不改动原树结构。
...
- **资源包编辑器 pack_format 版本查询接口（M4 pack_format 联动校验：后端）**（[explore.rs](src-tauri/src/commands/tools/resourcepack/explore.rs) / [explore_test.rs](src-tauri/src/commands/tools/resourcepack/explore_test.rs) / [types/resourcepack_explore.rs](src-tauri/src/commands/tools/types/resourcepack_explore.rs) / [dispatcher.rs](src-tauri/src/commands/tools/dispatcher.rs) / [mod.rs](src-tauri/src/commands/tools/resourcepack/mod.rs)）：新增 `rp_pack_format_info` action——输入 pack_format 数值，复用既有 `pack_format_to_version` 映射表返回对应 MC 版本范围描述与 `known` 标志（未知值 `known=false`），供前端表单输入时实时联动校验，避免前端重复维护版本映射表。含 1 项新单元测试（已知/未知 pack_format 返回值与 known 标志）。

- **资源包编辑器后端能力（M3 3D 模型预览：批量模型读取）**（[explore.rs](src-tauri/src/commands/tools/resourcepack/explore.rs) / [explore_test.rs](src-tauri/src/commands/tools/resourcepack/explore_test.rs) / [types/resourcepack_explore.rs](src-tauri/src/commands/tools/types/resourcepack_explore.rs) / [dispatcher.rs](src-tauri/src/commands/tools/dispatcher.rs) / [mod.rs](src-tauri/src/commands/tools/resourcepack/mod.rs)）：新增 `rp_read_many` action——以 blockstates/*.json 或 models/**/*.json 为入口，BFS 递归批量读取 3D 预览所需的整条引用链：blockstate 的 variants/multipart 模型引用 → 模型 JSON（跟随 `parent` 链，含命名空间补全：无命名空间引用优先当前文件命名空间、再补 `minecraft`）→ `textures` 中被引用的纹理 png（`#key` 内部引用跳过，base64 data URI 返回）；缺失的 vanilla parent / 关联文件 / 超限文件静默跳过不阻塞整批，仅入口 JSON 损坏时报错；路径安全复用 `resolve_in_work_dir`（`..` 跳转拦截 + canonicalize 限定工作目录内），文本 2MB / 图片 20MB 上限复用既有常量。返回 `{root, files: {relPath: {kind, content}}}` 供前端构建 lodestone Resources 渲染。含 3 项新单元测试（blockstate→parent→纹理全链收集 / 缺失 parent 与纹理静默跳过 / 非模型根文件拒绝）。

- **资源包编辑器 3D 模型预览（M3 前端）**（[RpModelPreview.vue](src/views/tools/data/RpModelPreview.vue)（新增） / [previewResources.ts](src/utils/resourcepack/previewResources.ts)（新增） / [ResourcePackEditor.vue](src/views/tools/data/ResourcePackEditor.vue) / [resourcepack.ts](src/utils/api/tools/resourcepack.ts) / [core.ts](src/utils/api/tools/core.ts) / [vite.config.ts](vite.config.ts)）：模型 / blockstate JSON 由纯文本编辑升级为 3D 预览——lodestone `ThreeStructureRenderer` + `Structure([1,1,1])` 渲染，拖拽旋转、滚轮缩放、自动旋转开关；入口模型解析：blockstate 取第一个模型引用（variants → multipart），模型文件直接渲染（合成 `""` 变体 blockstate）；新增 `buildPreviewResources` 构建合并 Resources——pack 模型/blockstate 优先、缺失回退内置原版资源并 flatten parent 链；纹理图集合并：pack 纹理覆盖同 id 原版区域、新 id 分配扩展区（画布幂次扩展）、未知 id 回退 16px 品红占位格；原版内置资源（atlas.png 1.47MB + assets.json 1.58MB + 4 个 flags 文件）经 Vite `?url` 静态导入，新增 `@mattzh72/lodestone/assets` 别名绕过包 exports 子路径限制。新增 `rpReadMany` API 封装与 `RP_READ_MANY` 常量；模型文件支持「3D 预览 ⇄ 编辑 JSON」切换，保留 M2 文本编辑能力。

- **资源包编辑器前端编辑能力（M2 编辑闭环）**（[ResourcePackEditor.vue](src/views/tools/data/ResourcePackEditor.vue) / [RpMcmetaForm.vue](src/views/tools/data/RpMcmetaForm.vue) / [RpTexturePreview.vue](src/views/tools/data/RpTexturePreview.vue) / [RpLangTable.vue](src/views/tools/data/RpLangTable.vue) / [RpTextEditor.vue](src/views/tools/data/RpTextEditor.vue)（新增） / [resourcepack.ts](src/utils/api/tools/resourcepack.ts) / [core.ts](src/utils/api/tools/core.ts)）：资源包编辑器由只读查看升级为可编辑——pack.mcmeta 表单支持编辑 pack_format 与描述（保留原 JSON 其他顶层字段，保存后包信息栏同步刷新）；语言文件表格支持键值单元格编辑、新增/删除条目与重复键校验后写回；JSON/模型/文本文件改为 textarea 编辑器（json/model 保存前先做语法校验，复用版本 JSON 编辑模式）；纹理预览新增「替换纹理」按钮（选择本地图片 → FileReader 转 base64 → `rp_write` 写回工作目录并即时刷新预览）。包信息栏新增「保存 ZIP」（zip 会话打包回原 zip，覆盖前二次确认）与「另存为 ZIP」（`pickSavePath` 选择目标路径后 `rp_export` 导出）；`src_path` 随打开结果返回供保存回原包。新增 `rp_write`/`rp_export` 类型安全 API 封装与 `TOOLS_ACTIONS` 常量。

- **资源包编辑器后端能力（M2 编辑闭环：写回与导出）**（[explore.rs](src-tauri/src/commands/tools/resourcepack/explore.rs) / [explore_test.rs](src-tauri/src/commands/tools/resourcepack/explore_test.rs) / [convert.rs](src-tauri/src/commands/tools/resourcepack/convert.rs) / [types/resourcepack_explore.rs](src-tauri/src/commands/tools/types/resourcepack_explore.rs) / [dispatcher.rs](src-tauri/src/commands/tools/dispatcher.rs) / [mod.rs](src-tauri/src/commands/tools/resourcepack/mod.rs)）：新增 `rp_write` action——按包内相对路径写回单文件（复用 `rp_read` 的路径安全校验）：`text` 类型直接写 UTF-8 文本，`base64` 类型解码二进制（允许带 data URI 前缀，解码后与文本同受大小上限约束），非法写入类型/越界路径返回失败；新增 `rp_export` action——打包导出 zip：zip 会话「保存」回原 zip 或「另存为」新路径，folder 会话另存为 zip，写临时文件成功后再原子替换目标（避免打包失败损坏原 zip）；打包复用 `convert::zip_directory` 并新增 `zip_directory_with_comment`（从源 zip 复制注释，保留 loader 附加属性）；`RpOpenResult` 新增 `src_path` 字段（zip 会话返回源 zip 路径，供保存回原包与注释复制）。含 3 项新单元测试（文本/base64 写回、越界路径拦截、导出 zip 往返且保留注释）。

- **资源包可视化编辑器前端界面（M1 查看器闭环）**（[ResourcePackEditor.vue](src/views/tools/data/ResourcePackEditor.vue)（新增） / [RpFileTreeNode.vue](src/views/tools/data/RpFileTreeNode.vue)（新增） / [RpMcmetaForm.vue](src/views/tools/data/RpMcmetaForm.vue)（新增） / [RpTexturePreview.vue](src/views/tools/data/RpTexturePreview.vue)（新增） / [RpLangTable.vue](src/views/tools/data/RpLangTable.vue)（新增） / [RpSoundPreview.vue](src/views/tools/data/RpSoundPreview.vue)（新增） / [resourcepack.ts](src/utils/api/tools/resourcepack.ts)（新增） / [core.ts](src/utils/api/tools/core.ts) / [tools.ts](src/utils/api/tools.ts) / [StoragePage.vue](src/views/tools/StoragePage.vue)）：存档资源新增「资源包编辑器」页签（页面自动切换 `max-w-6xl` 宽布局）。主容器支持从 resourcepacks 目录列表直接打开，或通过「打开 ZIP / 打开文件夹」按钮载入任意包；打开后展示包信息栏（pack.png 图标、ZIP/文件夹标记、pack_format、适用 MC 版本、描述、大小、文件数）与左文件树右内容分发：递归文件树（目录点击展开/收起，动画纹理紫色「动画」标记，文件按类型显示图标与大小，选中高亮）、pack.mcmeta 只读表单、纹理 PNG 2D 预览、语言文件键值表格、声音 OGG 试听、JSON/文本只读查看；打开新包时自动把上一会话 work_dir 回传后端清理临时目录；文件读取失败以 toast 提示。新增 `rp_open`/`rp_read` 类型安全 API 封装与 `TOOLS_ACTIONS` 常量。

- **资源包可视化编辑器后端能力（M1 查看器闭环）**（[explore.rs](src-tauri/src/commands/tools/resourcepack/explore.rs)（新增） / [explore_test.rs](src-tauri/src/commands/tools/resourcepack/explore_test.rs)（新增） / [types/resourcepack_explore.rs](src-tauri/src/commands/tools/types/resourcepack_explore.rs)（新增） / [dispatcher.rs](src-tauri/src/commands/tools/dispatcher.rs) / [packs/mod.rs](src-tauri/src/commands/version/packs/mod.rs)）：新增 `rp_open` action——打开资源包：zip 解压到临时工作目录（复用 `convert::unzip_to_dir` 安全解压）/ folder 直接使用原目录，返回包信息（名称/格式/大小/pack.png 图标 data URI/`pack.mcmeta` 的 pack_format 与 description）与完整结构树（目录在前，文件按类型分类 `mcmeta/png/model/lang/json/ogg/text/other`，动画纹理通过同名 `.png.mcmeta` 标记）；新增 `rp_read` action——按包内相对路径读单文件：png/ogg → base64 data URI 预览，json/lang/文本 → 原文。安全约束：读取路径拦截绝对路径与 `..` 跳转并 canonicalize 校验必须位于工作目录内，文本 2MB / 图片音频 20MB 大小上限，打开新包时自动清理上一 zip 会话的临时目录（仅限 temp 下 `molaunch-rp` 根内）；pack_format→MC 版本范围描述映射表（与 `skin_resourcepack/generate.rs` 的 `get_pack_format` 互补）；`packs/icon.rs` 的 icon 模块提升为 `pub(crate)` 供编辑器复用 `extract_pack_icon_data_url`。含 4 项单元测试（文件类型分类 / pack_format 映射 / 路径穿越拦截 / 结构树构建与动画标记）。

- **正版玩家皮肤获取后端能力**（[skin.rs](src-tauri/src/commands/tools/network/skin.rs)（新增） / [types/network.rs](src-tauri/src/commands/tools/types/network.rs) / [network/mod.rs](src-tauri/src/commands/tools/network/mod.rs) / [dispatcher.rs](src-tauri/src/commands/tools/dispatcher.rs)）：新增 `skin_fetch` action——输入正版玩家名，按 PCL2 同款 API 流程获取皮肤：`api.mojang.com/users/profiles/minecraft/{name}` 取 UUID → `sessionserver.mojang.com/session/minecraft/profile/{uuid}` 取 textures base64 → 解码出 `SKIN.url`（http 自动转 https）、`metadata.model`（slim/classic）与 `CAPE.url` → 下载皮肤/披风 PNG 转 base64 data URI 返回（前端直接预览）；玩家不存在（404）、未设置自定义皮肤等失败场景以 `error` 字段返回而非报错。新增 `skin_save_image` action——base64 写 PNG 文件到指定路径，带 PNG 魔数校验与 1MB 大小限制（参照 authlib 本地皮肤校验）。

- **常用工具新增「皮肤下载」子工具**（[SkinFetcher.vue](src/views/tools/network/SkinFetcher.vue)（新增） / [CommonPage.vue](src/views/tools/CommonPage.vue) / [network.ts](src/utils/api/tools/network.ts) / [core.ts](src/utils/api/tools/core.ts)）：常用工具分类新增「皮肤下载」页签——输入正版玩家名一键拉取官方皮肤：玩家信息卡片展示模型标签（slim 细手臂 / classic 粗手臂）与 UUID（8-4-4-4-12 格式，点击复制），皮肤/披风预览图可单独保存为 PNG（保存对话框 + 后端 `skin_save_image` 写文件）；玩家不存在、未设置自定义皮肤等失败场景在页面内以 Alert 说明原因，接口说明提示 Mojang 服务器稳定性。

- **常用工具「皮肤下载」改用 3D 皮肤模型预览**（[SkinFetcher.vue](src/views/tools/network/SkinFetcher.vue) / 复用 [SkinModel3D.vue](src/components/common/SkinModel3D.vue)）：皮肤获取结果由平面大图改为基于 skinview3d 的 3D 人物模型渲染——皮肤纹理自动挂载到模型并跟随 Slim（Alex）/ Classic（Steve）自动切换，有披风时同步挂载披风；支持鼠标拖动旋转 / 滚轮缩放与待机动画；右侧保留皮肤/披风小图与保存按钮，未设置披风时不再渲染披风区。

- **NBT 编辑器前端升级为可视化编辑与存档抽屉选文件**（[NbtViewer.vue](src/views/tools/data/NbtViewer.vue) / [NbtTreeNode.vue](src/views/tools/data/NbtTreeNode.vue) / [NbtSaveDrawer.vue](src/views/tools/data/NbtSaveDrawer.vue)（新增） / [data.ts](src/utils/api/tools/data.ts) / [core.ts](src/utils/api/tools/core.ts)）：NBT 数据页由「只读查看」升级为「可视化编辑」——树节点支持点击改值（数值/字符串/数组以逗号分隔文本编辑）、悬停新增子节点（命名 + 类型下拉，覆盖 byte/short/int/long/float/double/string/compound/list/byte_array/int_array/long_array）、列表按首个子类型新增元素、整节点删除，容器节点默认全展开；「保存」按钮按文件类型走 `nbt_save`（普通 NBT 直接写回，.mca 先选区块再写回该区块）；「从存档选择」按钮打开新增 `NbtSaveDrawer` 抽屉（参考种子地图 LoadSaveDrawer 的 版本 → 存档 → 文件 三级联动），展示 level.dat / playerdata / region .mca 等文件并按「存档数据/玩家数据/区块/其他」标注类别，选中即解析，免去逐层点文件夹；保存前二次确认并提示先备份。

- **NBT 数据编辑器后端支持保存、mca 区块解析与存档文件列表**（[nbt.rs](src-tauri/src/commands/tools/nbt.rs) / [nbt_test.rs](src-tauri/src/commands/tools/nbt_test.rs)（新增） / [types/nbt.rs](src-tauri/src/commands/tools/types/nbt.rs) / [dispatcher.rs](src-tauri/src/commands/tools/dispatcher.rs) / [archive/helpers.rs](src-tauri/src/commands/tools/archive/helpers.rs) / [archive/mod.rs](src-tauri/src/commands/tools/archive/mod.rs)）：NBT 查看升级为读写——新增 `nbt_save` action（普通 NBT 文件检测原 gzip 状态后保持 gzip 原子写回；.mca 按 Anvil 容器整体重打包写回指定区块，保留其他区块原字节与时间戳表），新增 `nbt_list_save_files` action（扫描存档目录内 level.dat / playerdata / region .mca 等 NBT 文件并返回相对路径/大小/类别/绝对路径）；`nbt_parse` 支持 .mca 解析（8KiB 头部位置表 + zlib/gzip/raw 区块数据，返回全部有效区块 `NbtChunkInfo`）；`NbtNode` 树与 `fastnbt::Value` 双向转换（`node_to_value` 为解析逆操作）；`archive::resolve_saves_dir` 提升为 pub(crate) 供 NBT 模块复用；含 4 项单元测试（树往返 / 空 compound / mca 解析与重打包 / 非法类型报错）。
...
- **关于页依赖清单与许可登记同步**（[about](src-tauri/resources/about)）：`frontend-deps.txt` 补充新增依赖 `country-flag-icons`（国旗图标）与 `pinyin-pro`（拼音转换）；`backend-deps.txt` 补充缺失的 `x509-parser` 0.17 并修正过时版本（Reqwest 0.12、rustls-native-certs 0.8、zip 4、rusqlite 0.40）；`frontend-dev-deps.txt` 修正过时版本（Vite ^6.4.3、@vitejs/plugin-vue ^6.0.8、Vitest ^3.2.7、@vue/eslint-config-typescript ^13.0.0）；`licenses.txt` 补登 pinyin-pro（MIT，zh-lx）、country-flag-icons（MIT，catamphetamine）、x509-parser（MIT/Apache-2.0，Rusticata）许可——以上均与 package.json / Cargo.toml 逐项核对一致。

- **物品搜索支持中文拼音全拼匹配**（[itemSearch.ts](src/utils/recipe-generator/itemSearch.ts)（新增） / [AchievementGenerator.vue](src/views/tools/creation/AchievementGenerator.vue) / [ItemPalette.vue](src/views/tools/creation/recipe-generator/ItemPalette.vue) / [ItemEditor.vue](src/views/tools/command/ItemEditor.vue)）：新增公共搜索工具 `matchItem`（新增依赖 `pinyin-pro`，中文名转无声调全拼并按中文名缓存），在原有 ID / 英文名 / 中文名子串匹配基础上加入拼音全拼匹配——如输入 `tiejian` 即可搜到「铁剑」；成就生成器、合成配方调色板、指令生成器三处物品搜索统一复用该工具，搜索框提示同步更新为「ID / 名称 / 中文 / 拼音」。
