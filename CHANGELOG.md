...
- **所有工具分类页重构为顶部子菜单切换，并移除右侧 TOC 悬浮导航**（[Tools.vue](src/views/Tools.vue) / [ArchivePage.vue](src/views/tools/archive/ArchivePage.vue) / [ModToolsPage.vue](src/views/tools/mod-tools/ModToolsPage.vue) / [NetworkPage.vue](src/views/tools/network/NetworkPage.vue) / [CalcPage.vue](src/views/tools/calc/CalcPage.vue) / [JavaPage.vue](src/views/tools/java/JavaPage.vue) / [DiagnosticPage.vue](src/views/tools/diagnostic/DiagnosticPage.vue) / [GameResourcePage.vue](src/views/tools/game-resource/GameResourcePage.vue) / [SeedMapPage.vue](src/views/tools/seedmap/SeedMapPage.vue) / [CreationPage.vue](src/views/tools/creation/CreationPage.vue) / [GradientTextPage.vue](src/views/tools/creation/GradientTextPage.vue) / [ExperimentalChat.vue](src/views/experimental/ExperimentalChat.vue) / [ToolToc.vue](src/components/common/ToolToc.vue)（删除））：存档管理 / Mod 工具 / 网络工具 / 计算工具 / Java 管理 / 诊断工具 / 游戏资源 / 种子地图八个分类页与创作、指令生成分类保持一致，由多工具垂直叠加改为顶部 `SubTabBar` 菜单栏（sticky 吸顶、带图标），每次只渲染当前工具，并统一支持 `?subtab=` 深链直达子工具；子工具锚点 `id` 保留供外部跳转。同时放弃 TOC：`Tools.vue` 移除 `ToolToc` 组件引用与 `tocRefreshKey` 重扫逻辑（外容器 padding 改为仅对直接在 Tools.vue 渲染的 3 个分类生效），`ExperimentalChat.vue` 删除对话目录 TOC 及配套 `tocTitle`/`tocRefreshKey` 代码，`CreationPage`/`GradientTextPage` 清理 `data-toc-*` 标注，删除 `ToolToc.vue` 组件。
