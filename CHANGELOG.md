...
- README 顶部品牌展示由静态 logo 更换为开屏动画 GIF：`images/splash.gif`（`scripts/capture-splash.mjs` 借助 Puppeteer 逐帧录制 splashscreen 动画，门闩机制确保从动画起点开始捕获，共 83 帧，Pillow 合成 640×180 循环播放，总时长约 4.15s），居中展示宽度 640，README 开头即可预览启动器开屏动画。
